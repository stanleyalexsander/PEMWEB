...
@section('title', 'Barang Keluar - Logistock')
                            ...
                            <div class="row m-0">
                                <div class="table-responsive">
                                    <table id="datatable" class="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                            @foreach($barangKeluarData as $barangKeluar)
                                                <tr>
                                                    <td>{{ $barangKeluar->kode_barang }}</td>
                                                    <td>{{ $barangKeluar->jumlah_keluar }}</td>
                                                    <td>{{ $barangKeluar->tanggal_keluar }}</td>
                                                    <td>{{ $barangKeluar->tujuan_barang }}</td>
                                                    <td class="text-center">
                                                        <a href="" class="btn btn-info text-color-base border-0 mx-1" data-bs-toggle="modal" data-bs-target="#modalEditData{{ $barangKeluar->id_keluar }}"><i class="fa-regular fa-pen-to-square"></i></a>
                                                        <a href="" class="btn btn-danger btn-delete text-color-base border-0 mx-1" data-id="{{ $barangKeluar->id_keluar }}"><i class="fa-regular fa-trash-can"></i></a>
                                                        <form action="{{ route('barang-keluar.destroy', $barangKeluar->id_keluar) }}" method="POST" id="delete-form-{{ $barangKeluar->id_keluar }}">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                                                    </td>
            ...
            <div class="modal-body py-0 px-4">
                ...
                <div class="row mx-0 mb-3">
                    ...
                    <select class="form-control select2" name="tujuan_barang" required>
                        ...
                        @foreach($barangKeluarData as $barangKeluar)
                            <option value="{{ $barangKeluar->tujuan_barang }}">{{ $barangKeluar->tujuan_barang }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
...
<!-- Modal Edit -->
@foreach($barangKeluarData as $barangKeluar)
    <div class="modal fade" id="modalEditData{{ $barangKeluar->id_keluar }}" data-bs-keyboard="false" tabindex="-1" aria-labelledby="modalEditDataLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header border-0 p-4">
                    ...
                    <div class="modal-body py-0 px-4">
                        <div class="row mx-0 mb-3">
                            ...
                            <select class="form-control select2" name="kode_barang" required>
                                <option value="{{ $barangKeluar->kode_barang }}" selected>{{ $barangKeluar->kode_barang }}</option>
                                @foreach($barangData as $barang)
                                    @if ($barangKeluar->kode_barang != $barang->kode_barang)
                                        <option value="{{ $barang->kode_barang }}">{{ $barang->kode_barang }}</option>
                                    @endif
                                @endforeach
                            </select>
                        ...
                        <div class="row mx-0 mb-3">
                            ...
                            <input class="form-control" type="number" name="jumlah_keluar" value="{{ $barangKeluar->jumlah_keluar }}" required>
                        </div>
                        <div class="row mx-0 mb-3">
                            <label class="text-color-text mb-2 mx-0 p-0">Tanggal Keluar</label>
                            <input class="form-control" type="date" name="tanggal_keluar" value="{{ $barangKeluar->tanggal_keluar }}" required>
                        </div>
                        <div class="row mx-0 mb-3">
                            <label class="text-color-text mb-2 mx-0 p-0">Tujuan Keluar</label>
                            <input class="form-control" type="text" name="tujuan_barang" value="{{ $barangKeluar->tujuan_barang }}" required>
                        </div>
                    </div>
                    <div class="modal-footer border-0 p-4">
                        <button type="button" class="btn bg-color-primary text-color-base me-4" data-bs-dismiss="modal">Batal</button>
