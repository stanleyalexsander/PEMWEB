...
@section('title', 'Pengguna - Logistock')
                                            ...
                                            @foreach($users as $user)
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <a href="" class="btn btn-info text-color-base border-0 mx-1" data-bs-toggle="modal" data-bs-target="#modalEditData{{ $user->id_pengguna }}"><i class="fa-regular fa-pen-to-square"></i></a>
                                                        <a href="" class="btn btn-danger btn-delete text-color-base border-0 mx-1" data-id="{{ $user->id_pengguna }}"><i class="fa-regular fa-trash-can"></i></a>
                                                        <form action="{{ route('pengguna.destroy', $user->id_pengguna) }}" method="POST" id="delete-form-{{ $user->id_pengguna }}">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                                                    </td>
...
<!-- Modal Edit -->
@foreach($users as $user)
<div class="modal fade" id="modalEditData{{ $user->id_pengguna }}" data-bs-keyboard="false" tabindex="-1" aria-labelledby="modalEditDataLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header border-0 p-4">
        ...
        <form action="{{ route('pengguna.update', $user->id_pengguna) }}" method="POST">
            ...
            <div class="modal-body py-0 px-4">
                ...
                <div class="row mx-0 mb-3">
                    <label class="text-color-text mb-2 mx-0 p-0">Nama</label>
                    <input class="form-control" type="text" name="nama_pengguna" value="{{ $user->nama_pengguna }}" required>
                </div>
